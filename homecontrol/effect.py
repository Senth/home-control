...
class Effects:
    ...
    def test_effect():
        ...
        effect.add_transition(ColorTransition(x_start, y_start, 0))
        effect.add_transition(ColorTransition(x_stop, y_stop, 100))
...
class Transition:
    ...
    def run(self, light_or_group):
        logger.debug("Transition.run() Nothing running, transition_time: " + str(self.transition_time))
...
class ColorTransition(Transition):
    ...
    def run(self, light_or_group):
        ...
        TradfriGateway.color(light_or_group, self.x, self.y, transition_time=self.transition_time)
...
class BrightnessTransition(Transition):
    ...
    def run(self, light_or_group):
        logger.debug("BrightnessTransition.run() Transitioning to light level {} in {}".format(
