...
class Transition:
    ...
    def run(self, light_or_group):
        logger.debug(
            "Transition.run() Nothing running, transition_time {} seconds ".format(self.transition_time))
...
class ColorTransition(Transition):
    ...
    def run(self, light_or_group):
        logger.debug("ColorTransition.run() Transitioning to ({}, {}) in {}".format(
            self.x, self.y, self.transition_time
        ))
        TradfriGateway.color(light_or_group, self.x, self.y,
...
class BrightnessTransition(Transition):
    ...
    def run(self, light_or_group):
        logger.debug("BrightnessTransition.run() Transitioning to light level {} in {} seconds".format(
            self.brightness, self.transition_time
        ))
        TradfriGateway.dim(light_or_group, self.brightness,
...
class BrightnessColorTransitionFactory:
    ...
    @ staticmethod
    def create(brightness_from, brightness_to, color_x_from, color_x_to, color_y_from, color_y_to, transition_time):

        part_count = transition_time / \
            BrightnessColorTransitionFactory.TRANSITION_TIME_PART / 2
        ...
        for i in range(part_count):
            ...
            x = color_x_parts[i]
            y = color_y_parts[i]
            transitions.append(ColorTransition(
            ...
            transitions.append(BrightnessTransition(
